...
export const ReceipeCard = ({ item, index, hideFav }: any) => {
    ...
    const handleFavorites = (key: number,idRecipe:any) => {
        ...
        if (getUser[0].favorites.includes(getRecipe[0].recipeId)) {
            let filterIndex = getUser[0].favorites.indexOf(idRecipe)
            getUser[0].favorites.splice(filterIndex, 1)
    ...
    return (
        <div className="slider">
            ...
            <div className="slider-caption d-flex justify-btn align-center">
                ...
                {!hideFav &&
                    <button className={favorites?.includes(item.recipeId) ? "active" : ''} type="button" onClick={() => handleFavorites(index, item.recipeId)}>
